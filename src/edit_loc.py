...
class edit_loc(QtWidgets.QDialog):
    def __init__(self):
        ...
        uic.loadUi(r'ui\edit_loc.ui', self)
        self.setWindowIcon(QtGui.QIcon(r'icon\icon.png'))
        self.setWindowTitle("Asset Manager - Edit Location")
    ...
    def fetch(self, text):
        global_variable.mycursor.execute("SELECT active, floor, wing, name from location where name like '%"+text+"%';")
    ...
    def generate(self):
        global_variable.mycursor.execute("SELECT active, floor, wing, name from location;")
        data = global_variable.mycursor.fetchall()
        self.log.setRowCount(0)
        if len(data) == 0:
            return 
